...
for j in range(nLocations):
    for k in range(nLocations):
        for m in range(nLocations):
            for n in range(nLocations):
                objective += flow_matrix[j][k] * distance_matrix[m][n] * x[j][m] * x[k][n]
...
for i in range(nLocations):
    cqm.add_constraint(np.sum([x[j][i] for j in range(nLocations)]) == 1)
